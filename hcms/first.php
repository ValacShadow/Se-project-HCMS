...
<div class="topnav">
  ...
  <a href="student.php">Register complaint</a>
...
<section>
<aside class="left">
<h2>Previous Complaint</h2>
<?php 
   $conn = mysqli_connect('localhost','root','','hcms');
   $sql = "SELECT * FROM complaint WHERE username='$user' and status='solved'";
   ...
   while ($row =mysqli_fetch_assoc($result)){
	   ...
	   echo "<p>Complaint id: ".$row['id']."</p>";
		...
		echo "<p>Date and time:".$row['time_stamp']."</p>";
	    echo"</div>";		
   }
   ...
   <aside class="right">
<h2>Current Complaint status</h2>
<?php 
   $conn = mysqli_connect('localhost','root','','hcms');
   $sql = "SELECT * FROM complaint WHERE username='$user' and status='pending'";
   $result=mysqli_query($conn,$sql);
   while ($row =mysqli_fetch_assoc($result)){
   ...
   ?>   
<a href="student.php" <button style="width:auto;align:center;">Register a Complaint</button></a>
   </aside>
</section>
</body>
